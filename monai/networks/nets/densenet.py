...
class EMA(nn.Module):
    ...
    def forward(self, x):
        ...
        x21 = self.softmax(self.agp(x2).reshape(b * self.groups, -1, 1, 1, 1).permute(0, 4, 2, 3, 1))
...
class DenseNet121_EMA(DenseNet_ema):
    """DenseNetEMA with optional pretrained support when `spatial_dims` is 2."""
...
Densenet121_EMA = densenet121_EMA = DenseNet121_EMA
